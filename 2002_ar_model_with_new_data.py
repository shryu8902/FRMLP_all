...
#%%
params = {'AR-TRNSFMR':{'d':256, 'n':2, 'h':128, 'lr':0.001, 'd_emb':32}}
C_IN = 48
D_OUT = 7
D_IN = C_IN + D_OUT
OUT_LEN = 333
...
for R_SEED, (tr_ind, te_ind) in enumerate(skf.split(range(len(np_scaled_inputs)), np_index[:,0])):
    if R_SEED == 0:
        ...
        new_x_val, new_y_val = train_concatenator(x_tr, y_tr)
            ...
            for model_name in ['AR-TRNSFMR']:
                ...
                for x_te_batch in list_x_te_bathces:
                    ...
                    for i in range(OUT_LEN):
                        ...
                        out, _ = current_model(torch.Tensor(temp_x_te_input).cuda())
