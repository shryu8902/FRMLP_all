...
# from Model.REGR_TRNSFMR import AR_TRANSFMR
...
loader_train = DataLoader(train, batch_size = 16, shuffle =True)
loader_test = DataLoader(test, batch_size= BATCH, shuffle = False)
#%%
params = {'AR-TRANSFMR':{'d':256, 'n':2, 'h':128, 'lr':0.001, 'd_emb':32}}
model_name = 'AR-TRANSFMR'
OUT_LEN=333
C_IN = 55
current_model = AR_TRANSFMR(c_in = C_IN, d_model = params[model_name]['d'], d_emb = params[model_name]['d_emb'],
            n_out = 7, out_len = OUT_LEN, n_layers = params[model_name]['n'], n_head = 8, loss_weight=[1,1,1,1,1,1,1],
            dr_rates = 0.2, use_lr_scheduler = False, lr =params[model_name]['lr'])
...
trainer = pl.Trainer(gpus = 1, max_epochs = 10, progress_bar_refresh_rate = 2)
trainer.fit(current_model, loader_train)
input_vec_te = x_te[:16,:]

for i in range(10):
    input_seq = np.tile(input_vec_te[:,np.newaxis,:],(1,i+1,1))
    if i ==0:
        strt_tkn = -2*np.ones((16,1,7))
        outwtkn = strt_tkn
    x_te_input = np.concatenate((input_seq, outwtkn),axis=-1)
    # outwtkn = np.concatenate((strt_tkn, output_seq), axis = 1)[:,:-1,:]
    # new_input = np.concatenate((input_seq, outwtkn), axis = -1)
    out, _ = current_model(torch.Tensor(x_te_input).cuda())
